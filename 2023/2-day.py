input_str = """Game 1: 14 green, 8 blue, 9 red; 5 blue, 4 green, 2 red; 4 red, 4 blue, 4 green; 1 blue, 3 green, 2 red; 10 red, 3 blue, 15 green; 2 red, 6 green, 3 blue
...
def possible_games(red, blue, green):
    ...
    for game in games:
        flag = True
        power = {'red': 1, 'blue': 1, 'green': 1}

        while flag:
            name, level = game.split(': ')
            _, no = name.split()
            ...
            for lvl in levels:
                shows = lvl.split(', ')
                shows = [i.split() for i in shows]
                shows = [(int(i[0]),i[1]) for i in shows]
                showed = {'red': 0, 'blue': 0, 'green': 0}


                for n, color in shows:
                    if color == 'red':
                        showed['red'] += n
                        power['red'] = max(power['red'], n)
                    if color == 'blue':
                        showed['blue'] += n
                        power['blue'] = max(power['blue'], n)
                    if color == 'green':
                        showed['green'] += n
                        power['green'] = max(power['green'], n)

                # part 1 puzzle
                # if showed['red'] > red or showed['blue'] > blue or showed['green'] > green:
                #     flag = False
        ...
        if flag:
            ...
            print(no)
            flag = False
        sum_of_power += power['red'] * power['blue'] * power['green']

    return total, sum_of_power


if __name__ == "__main__":
    res = possible_games(12, 14, 13)
    print(res)