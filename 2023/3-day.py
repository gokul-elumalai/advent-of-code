...
def is_valid(lines, li, si, n):
    line = lines[li]
    if si > 0 and line[si - 1] != '.':
        return True
    if si + n < len(line) - 1 and line[si + n] != '.' and not line[si + n].isdigit():
        return True

    if li > 0:
        top_line = lines[li - 1]
        if check_adj(top_line, si, n):
            return True
    if li < len(lines) - 1:
        bottom_line = lines[li + 1]
        if check_adj(bottom_line, si, n):
            return True
...
def calculate_sum():
    ...
    for i in range(len(lines)):
        ...
        for j in range(len(line)):
            ...
            if ch.isdigit():
                num_str = get_num(line, j)

                if is_valid(lines, i, j, len(num_str)):
                    # print(num_str)
                    total += int(num_str)
