...
input_str = """.........232.633.......................803..........................361................192............539.................973.221...340.....
...
def get_gear_ratio(lines, li, si):
    line = lines[li]
    gears = []

    if si -1 >= 0 and line[si-1].isdigit(): # left
        gears.append(get_gear(line, si-1))
    if si+1 < len(line) and line[si+1].isdigit(): # right
        gears.append(get_gear(line, si+1))

    if li-1 >= 0:
        line1 = lines[li-1]
        if line1[si].isdigit():
            gears.append(get_gear(line1, si))
        else:
            if si - 1 >= 0 and line1[si - 1].isdigit():  # left
                gears.append(get_gear(line1, si - 1))
            if si + 1 < len(line1) and line1[si + 1].isdigit():  # right
                gears.append(get_gear(line1, si + 1))

    if li+1 < len(lines):
        line2 = lines[li+1]
        if line2[si].isdigit():
            gears.append(get_gear(line2, si))
        else:
            if si - 1 >= 0 and line2[si - 1].isdigit():  # left
                gears.append(get_gear(line2, si - 1))
            if si + 1 < len(line2) and line2[si + 1].isdigit():  # right
                gears.append(get_gear(line2, si + 1))
...
def get_gear(line, idx):
    s = idx
    e = idx

    while s-1>=0 and line[s-1].isdigit():
        s -= 1

    while e+1 < len(line) and line[e+1].isdigit():
        e += 1

    return line[s:e+1]


def check_adj(line, idx, n):
    for i in range(idx, idx + n):
        if i < len(line) and line[i] != '.' and not line[i].isdigit():
            return True

    if idx > 0 and line[idx - 1] != '.' and not line[idx - 1].isdigit():
        return True
    if idx + n < len(line) and line[idx + n] != '.' and not line[idx + n].isdigit():
        return True
...
def calculate_sum():
    ...
    for i in range(len(lines)):
        ...
        for j in range(len(line)):
            ...
            if ch == '*':
                total_gear_ratio += get_gear_ratio(lines, i, j)
